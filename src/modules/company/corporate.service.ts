...
export class CorporateService {
    ...
    async createCorporate(dto: CreateCorporateDto) {
        try {
            ...
            if (existsing) {
                return standardResponse(false, 'Corporate already exists', 404, null, null, 'corporate/create');
            }
            ...
            return standardResponse(true, 'Corporate created successfully', 201, saved, null, 'corporate/create');
        } catch (err: any) {
            return standardResponse(false, 'Corporate already exists', 409, null, err?.message, 'corporate/create');
        }
    }
    ...
    async updateCorporate(dto: UpdateCorporateDto) {
        ...
        if (!corporate) {
            return standardResponse(false, 'Corporate not found', 404, null, null, 'corporate/update');
        }
        const duplicateCheck = await this.corporateRepo.findOne({ where: { corporateName: dto.corporateName } });
        if (duplicateCheck && duplicateCheck.id !== id) {
            return standardResponse(false, 'Corporate name already in use', 409, null, null, 'corporate/update');
        }
        const wasActive = corporate.isActive;
